...
func nonCompliant3(w http.ResponseWriter, r *http.Request){
    session, err := store.Get(r, "blah-session")
   var user_id int = session.Values["user_id"].(int)
   ...
   // ruleid: rule-handler-assignment-from-multiple-sources
   user_id = r.someMap[user_id]
   user_obj := RetrieveUser(user_id)
   user_obj.account_id = r.query.params.account_id
   user_obj.save()
}
func nonCompliant4(w http.ResponseWriter, r *http.Request){
    session, err := store.Get(r, "blah-session")
   var user_id int = session.Values["user_id"].(int)
...
func (sc *http.serverConn) compliant2(rw *http.responseWriter, req *http.Request, handler func(http.ResponseWriter, *http.Request)) {
        // ok: rule-handler-assignment-from-multiple-sources
	didPanic := true
	defer func() {
