...
func taintedSqlStringNoncompliant(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	defer db.Close()
	// Noncompliant: query contains unvalidated user input.
...
func taintedSqlStringCompliant(ctx context.Context, name MyEvent) (string, error) {
	...
	defer db.Close()
	// Compliant: query contains input from context.
...
func main() {
	lambda.Start(taintedSqlStringNoncompliant)
	lambda.Start(taintedSqlStringCompliant)
}